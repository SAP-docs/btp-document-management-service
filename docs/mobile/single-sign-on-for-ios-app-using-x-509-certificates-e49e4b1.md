<!-- loioe49e4b1e6fdd4550be47c6d497da6901 -->

# Single Sign-On for iOS App Using X.509 Certificates

You can configure your iPad or iPhone SAP Document CenterSAP Mobile Documents app with a certificate for logging on without a user name and password.
...
-   You have installed the SAP Document CenterSAP Mobile Documents app on your iOS device.
-   You have hooked up your device to iTunes.



## Procedure

1.  Export your X.509 certificate with your private key from your key chain \(on Mac OS with file extension .p12\) or from your Internet Explorer \(on Microsoft Windows with file extension .pfx\). Choose a secure password to protect the file.

2.  Using iTunes, import the certificate into your *Mobile Docs**Doc Center* Documents folder.

3.  Launch your SAP Document CenterSAP Mobile Documents app.

4.  On the *Account* page, enter the server URL.

    The server checks whether certificate logons are allowed and, if so, displays the dialog for importing the certificate.

5.  Enter the password you chose in step 1.

    > ### Note:  
    > If you import the certificate after entering the server URL in the account settings of the app, you might get an authentication error message. In this case, restart the app. To do so, close the app by double-tapping the home button and flipping up the app's page. Then start the app again by tapping the icon \(![](images/iPad_Start_Icon_dbcd07e.png)\).
...
The X.509 certificate is stored on the device protected with your app passcode, and is used to log you on to the SAP Document CenterSAP Mobile Documents server.
